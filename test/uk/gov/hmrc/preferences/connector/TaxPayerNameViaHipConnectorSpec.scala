...
import com.github.tomakehurst.wiremock.client.WireMock.{ aResponse, absent, get, givenThat, urlEqualTo }
...
//ToDo Refactor the tests to TaxPayerNameConnector once hip endpoint is enabled
class TaxPayerNameViaHipConnectorSpec
  ...
  private val sautr: SaUtr = SaUtr("2000029888")
    ...
    "return TaxpayerName when retrieving a full taxpayer details payload" in new TestCase {
      private val jsonBody =
        """{
          ...
          | """.stripMargin
      givenThat(
        get(urlEqualTo("/ods-sa/v1/self-assessment/individual/2000029888/designatory-details/taxpayer"))
          .withHeader("Authorization", WireMock.notMatching(""))
          .withHeader("correlationId", WireMock.notMatching(""))
          .willReturn(
            aResponse()
    ...
    "return TaxpayerName when retrieving a partial taxpayer details payload which includes a full name object" in new TestCase {
      private val jsonBody =
        """{
          ...
          | """.stripMargin
      givenThat(
        get(urlEqualTo("/ods-sa/v1/self-assessment/individual/2000029888/designatory-details/taxpayer")).willReturn(
          aResponse()
            .withStatus(Status.OK)
            .withBody(jsonBody)
        )
