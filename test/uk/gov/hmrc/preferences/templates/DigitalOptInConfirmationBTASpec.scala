...
class DigitalOptInConfirmationBTASpec extends PlaySpec with MockitoSugar {

  private def expectedHtml(): String =
  ...
  "The digitalOptInConfirmation_BTA template" should {
    ...
    "render an HTML in English" in {
      val content = html.digitalOptinConfirmation()
      content.toString().filter(_ >= ' ') mustEqual expectedHtml()
    }

    "render an HTML in Welsh" in {
      val content = html.digitalOptinConfirmation_cy()
      content.toString() must include(
        "<p class=\"govuk-body\">Gan eich bod wedi cofrestru i gael llythyrau treth ar-lein erbyn hyn, rydym am roi rhagor o wybodaeth i chi.</p>"
      )
    }
  }
}
